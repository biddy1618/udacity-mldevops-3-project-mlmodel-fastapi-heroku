...
def clean_data():
    ...
    logger.info(f'Number of rows containing `?`: {df.apply(_find_question_mark, axis=1).sum()}')
    df.replace({'?': 'Unknown'}, inplace = True)
    logger.info(f'Number of rows containing `?` after replacement: {df.apply(_find_question_mark, axis=1).sum()}')
